...
const {
    ...
    changeEmail,
    editCard,
} = require('./controller/fsController');
...
try {
    const key = fs.readFileSync(
        '/etc/letsencrypt/live/xn--80aimkbegibhlk6a3ixb.xn--p1ai/privkey.pem'
    );
    const cert = fs.readFileSync(
        '/etc/letsencrypt/live/xn--80aimkbegibhlk6a3ixb.xn--p1ai/fullchain.pem'
    );
    ...
    try {
        ...
        if (deleteData.length > 0) {
            for (let item in deleteData) {
                ...
                if (fs.existsSync(filePath)) {
                    fs.unlinkSync(
                        `images/modulTowers/${directory[0]}/${directory[1]}/${directory[2]}`
                    );
                } else {
                    fs.unlinkSync(
                        `images/modulTowers/${directory[0]}/${directory[1]}_${data.id}/${directory[2]}`
                    );
                }
            }
        }

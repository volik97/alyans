...
function Header() {
    ...
    return (
        <>
            <Navbar
                placeholder={''}
                className='fixed top-0 z-20 border-none rounded-none max-w-full bg-[rgba(47,46,51,0.90)] py-3 px-5 lg:py-[15px] lg:px-[42px ]'
            >
                <div className='h-full flex items-center justify-between'>
                    <div className='w-full flex justify-between lg:gap-10 items-center py-[7px]'>
                        ...
                        <div className={'hidden lg:flex'}>{classicMenu}</div>
                    </div>
                </div>
            </Navbar>
            ...
            <div
                className={`fixed top-[62px] shadow-lg right-0 h-full w-80 bg-[rgba(47,46,51,0.90)] py-20 px-8 z-30 backdrop-saturate-200 backdrop-blur-2xl transform transition-transform duration-300 ease-in-out lg:hidden ${openNav ? 'translate-x-0' : 'translate-x-full'}`}
            >
                <div className='flex flex-col h-full'>{classicMenu}</div>
            </div>
        </>
    );
}

...
function Email() {
    ...
    useEffect(() => {
        axios.get(`https://${url}/getEmail`).then(response => {
    ...
    return (
        <div className={'flex flex-row p-40 justify-center items-start gap-x-20 text-white'}>
            <div className={'text-2xl'}>
                Текущий E-mail <a className={'bg-green-600 py-1 px-5'}>{email}</a>
            </div>
            <form onSubmit={handleSubmit} className={'flex flex-col text-2xl gap-3'}>
                ...
                <button
                    type='submit'
                    className={'bg-red-600 self-center mt-5 w-fit px-6 py-2 hover:bg-red-800'}
                >
                    Изменить
                </button>
            </form>
